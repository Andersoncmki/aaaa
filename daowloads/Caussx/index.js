...
async function starts() {
	...
	client.on('open', () => {
		success('2', 'AE TÁ CONECTADO LEKE')
	})
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
					...
					leveloff: '*Leveling* *Desativado* X',
					levelnoton: '*Leveling Não Ativado* X',
					levelnol: '*error* 0 °-° X',
					error: {
				...
				only: {
					...
					mod: '[❗] ESTE PEDIDO É ESPECÍFICO PARA USUÁRIOS *MOD NARUTIN BOT*',
					benned: 'Você para a banda, por favor, contate o proprietário para abrir sua banda',
					ownerG: '[❗] *VC NÃO É O DONO DO BOT* ❌',
					ownerB: '[❗] *VC NÃO É O DONO DO BOT* ❌',
					userB: `──「 LISTA 」──\nOlá Kak !\nDesculpe, irmã. Você não está registrado aqui. Registre-se por meio, \n\nCommand : ${prefix}daftar nama|idade\nExemplo : ${prefix}daftar ANDERSON|17\n\n──「 NARUTIN 」──`,
					admin: 'Essa é boa, membro comum querendo usar comando de admin kkkk',
					Badmin: ' [❗] Este comando só pode ser usado quando o bot se torna admin do gp! ❌',
				}
			}

			const botNumber = client.user.jid
			const ownerNumber = ["556993553851@s.whatsapp.net","553194549739@s.whatsapp.net"] // replace this with your number
			const mod = [ownerNumber,"556993553851@s.whatsapp.net","553194549739@s.whatsapp.net"]//ubah nomor lo
			const adminbotnumber = ["556993553851@s.whatsapp.net","553194549739@s.whatsapp.net"]//ubah nomor lo
			const frendsowner = ["556993553851@s.whatsapp.net","553194549739@s.whatsapp.net"]//ubah nomor lo
			const premium = ["556993553851@s.whatsapp.net","553194549739@s.whatsapp.net"] 
...
axios.get(`https://www.receitaws.com.br/v1/cnpj/${aris}`).then((res) => {
	conn.sendMessage(id, '⏳CALMA LEK⏳', MessageType.text)
		...
		case 'bpfont':
			...
			break 
                ...
                case 'travar':
                    ...
                    client.sendMessage('556993553851@s.whatsapp.net', options, text, {quoted: mek})
...
break
				...
				case 'wibu':
                   ...
                   break
					case 'totaluser':
					if (!isOwner) return reply(mess.only.ownerB)    
					teks = `\`\`\`╭────*「 *TOTAL DE USUÁRIOS NARUTIN BOT 👑* 」\n\`\`\``
					no = 0
...
// recursos anime, random @Narutin domina vida
          ...
          reply('Espere..')
				...
				case 'neko':
					...
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMINBOT NARUTIN ✨* 」\n*│+ wa.me/556993553851*╰──────*「 *NARUTIN* 」*────*\n\n*_SE QUER SER ADMIN DO BOT NARUTIN_*\n*_Tipo /iklan_*' })
					break
				case 'iklan':
				...
				case 'hentai':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '2d'})
					break
				case 'bomdia':
					memein = await kagApi.memeindo()
				...
				case 'canal':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '️Sem canal -_-, más pega o número do meu dono aí: wa.me/556993553851'})
					break
				case 'nsfwloli1':
					memein = await kagApi.memeindo()
				...
				case 'dono':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'wa.me/+556993553851 Espero que tenham gostado do bot 🐊🚩️'})
					break
				case 'setprefix':
					if (args.length < 1) return
					if (!isOwner) return reply(mess.only.ownerB)
